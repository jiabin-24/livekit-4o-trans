...
def build_stt():
    """Azure OpenAI gpt-4o-transcribe STT (HTTP/REST, 整句返回)."""
    common = dict(
    ...
    if _use_key_auth():
        return openai.STT.with_azure(api_key=os.getenv("AZURE_OPENAI_API_KEY"), **common)
    return openai.STT.with_azure(azure_ad_token=token_manager.get_token(), **common)
...
def build_tts():
    ...
    # Entra ID auth
    return azure.TTS(
        speech_auth_token=token_manager.get_token(),
        speech_region=speech_region,
        voice=voice,
    )
